...
const KEY_FILE_LOCATION = `${GLib.get_user_data_dir()}/secrets/gemini_key.txt`;
const APIDOM_FILE_LOCATION = `${GLib.get_user_data_dir()}/secrets/gemini_dom.txt`;
...
class GeminiService extends Service {
    ...
    send(msg) {
        ...
        const message = new Soup.Message({
            ...
            uri: GLib.Uri.parse(
                replaceapidom(
                    `https://generativelanguage.googleapis.com/v1/models/gemini-pro:streamGenerateContent?key=${this._key}`,
                ),
                GLib.UriFlags.NONE,
            ),
        });
